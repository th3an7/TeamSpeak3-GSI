...
const char* ts3plugin_name() {
	...
	if(!result) {
		const wchar_t* name = L"Aurora GSI Integration";
		if(wcharToUtf8(name, &result) == -1) {  /* Convert name into UTF-8 encoded result */
			result = "Aurora GSI Integration";  /* Conversion failed, fallback here */
		}
	}
	return result;
#else
...
int send_state(void) {
	...
	if (curl) {
		...
		curl_easy_setopt(curl, CURLOPT_URL, "localhost:9088");
		/* Now specify the POST data */
		curl_easy_setopt(curl, CURLOPT_POSTFIELDS, "state=state&key=effect");
...
int ts3plugin_onTextMessageEvent(uint64 serverConnectionHandlerID, anyID targetMode, anyID toID, anyID fromID, const char* fromName, const char* fromUniqueIdentifier, const char* message, int ffIgnored) {
	...
	char *nickname;
	if (ts3Functions.getClientSelfVariableAsString(serverConnectionHandlerID, CLIENT_NICKNAME, &nickname) == ERROR_ok) {
		if (fromName != nickname) {
			printf("PLUGIN: onTextMessageEvent: Received text message - nickname: %s - fromName: %s\n", nickname, fromName);
